  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col items-center animate-fade-in">
      ...
      <div className="flex items-center gap-6 mt-8">
        <button onClick={prevCard} className="w-12 h-12 rounded-full bg-white border border-slate-200 shadow-sm flex items-center justify-center hover:bg-slate-50 transition-colors focus:outline-none"><i className="fas fa-arrow-left text-slate-600"></i></button>
        <span className="font-semibold text-slate-500 text-lg">{currentIndex + 1} / {data.flashcards.length}</span>
        <button onClick={nextCard} className="w-12 h-12 rounded-full bg-white border border-slate-200 shadow-sm flex items-center justify-center hover:bg-slate-50 transition-colors focus:outline-none"><i className="fas fa-arrow-right text-slate-600"></i></button>
      </div>
    </div>
  );
}
